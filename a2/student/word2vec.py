...
def naiveSoftmaxLossAndGradient(
    ...
    ### to integer overflow.
    logits = np.dot(outsideVectors, centerWordVec)
    probabilities = softmax(logits)
    loss = -np.log(probabilities[outsideWordIdx])

    d_score = probabilities.copy()
    d_score[outsideWordIdx] -= 1
    gradCenterVec = np.dot(outsideVectors.T, d_score)
    gradOutsideVecs = np.outer(d_score, centerWordVec)
...
def negSamplingLossAndGradient(
    ...
    outsideVectors_ = outsideVectors[indices]
    logits = np.dot(outsideVectors_, centerWordVec)
    sig_pos = sigmoid(logits[0])
    sig_neg = sigmoid(-logits[1:])
    loss = -np.log(sig_pos) - np.sum(np.log(sig_neg))

    gradCenterVec = (sig_pos - 1)*outsideVectors_[0]
    for k in range(1, K+1):
        gradCenterVec += sigmoid(logits[k]) * outsideVectors_[k]

    gradOutsideVecs = np.zeros_like(outsideVectors)
    gradOutsideVecs[outsideWordIdx] += (sig_pos - 1)*centerWordVec
    for i,j in enumerate(negSampleWordIndices):
        gradOutsideVecs[j] += sigmoid(logits[i + 1]) * centerWordVec
...
def skipgram(currentCenterWord, windowSize, outsideWords, word2Ind,
    ...
    for outsideWord in outsideWords:
        ...
        if word2vecLossAndGradient == naiveSoftmaxLossAndGradient:
            single_loss, grad_v_c, grad_u = naiveSoftmaxLossAndGradient(v_c, outsideWordIdx, outsideVectors,dataset)
            loss += single_loss
            gradCenterVecs[current_index] += grad_v_c
            gradOutsideVectors += grad_u
        else:
            single_loss, grad_v_c, grad_u = negSamplingLossAndGradient(v_c, outsideWordIdx, outsideVectors,dataset)
            loss += single_loss
            gradCenterVecs[current_index] += grad_v_c
            gradOutsideVectors += grad_u
    ### END YOUR CODE
